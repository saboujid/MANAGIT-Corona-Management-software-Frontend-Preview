...
(
    ...
    CONSTRAINT ensure_contact
        CHECK(NOT(email IS NULL AND phone_number IS NULL))
);
...
(
    PlaceID INT NOT NULL, -- primary key column
...
-- Place Owners have their "Place" referenced by a foreign key
-- The do not have any login functionality
CREATE TABLE IF NOT EXISTS PlaceOwners
(
    PlaceOwnerID INT NOT NULL, -- primary key column
    PlaceOwnerName VARCHAR(128) NOT NULL,
    PlaceOwnerPhoneNumber VARCHAR(20),
    PlaceOwnerEmail VARCHAR(64),
    
    -- Place that the owner owns
    PlaceOwnedID INT NOT NULL,
    FOREIGN KEY (PlaceOwnedID) REFERENCES Places(PlaceID),
...
(
    AgentID INT NOT NULL, -- primary key column

    -- login details
    AgentUsername VARCHAR(64),
    AgentPassword VARCHAR(256),
...
(
    HospitalID INT NOT NULL, -- primary key column
...
(
    OccurrenceID INT NOT NULL, -- primary key column
